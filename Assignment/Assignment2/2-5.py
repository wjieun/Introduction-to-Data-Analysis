...
# location_df['estim_temp'] 채워넣을것
date_time = input()
df1 = weather_df[weather_df['date_time'] == date_time]
df1 = df1[['station_name', 'temperature']]

distance_df['station_name'] = distance_df.idxmin(axis=1)
location_df = pd.merge(location_df, distance_df['station_name'], left_on='name', right_index=True)
location_df = pd.merge(location_df, df1, on='station_name')
location_df = location_df.set_index('name')
location_df['estim_temp'] = location_df['temperature']

check_location = input()
print(f"{location_df.loc[check_location, 'estim_temp']:.2f}°C")
show_temp_map = True  # 시각화 여부 결정. False로 할 시 실행 시간을 줄일 수 있음.
if show_temp_map:
	# 한국 지도 
	...
	# 읍/면/동 예상 온도 찍기. 온도를 색으로 사용.
	plt.scatter(location_df['longitude'], location_df['latitude'], 
							c=location_df['estim_temp'], cmap='coolwarm', marker='o', alpha=1.0)
